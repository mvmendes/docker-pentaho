...
MAINTAINER Marcus Mendes
# forked and base from ca0abinary/docker-pentaho (Jonathan DeMarks)
...
ENV CATALINA_OPTS="-Djava.awt.headless=true -Xms4096m -Xmx6144m -XX:MaxPermSize=256m -Dsun.rmi.dgc.client.gcInterval=3600000 -Dfile.encoding=utf8 -Dsun.rmi.dgc.server.gcInterval=3600000"
...
# Get Pentaho Server
RUN echo https://ufpr.dl.sourceforge.net/project/pentaho/Pentaho%20${MAJOR_VERSION}/server/pentaho-server-ce-${MINOR_VERSION}.zip | xargs wget -qO- -O tmp.zip && \
...
# Get Mysql JDBC driver for replace outdated  version
RUN echo https://cdn.mysql.com/archives/mysql-connector-java-5.1/mysql-connector-java-5.1.48.zip | xargs wget -qO- -O tmp.tar.gz && \
    tar -zxf tmp.tar.gz && \
	rm -f tmp.tar.gz && \
	cp mysql-connector-java-5.1.48/mysql-connector-java-5.1.48.jar ${PENTAHO_SERVER}/tomcat/lib/ && \
	rm -fr mysql-connector-java-5.1.48

# Remove outdated Mysql JDBC driver
RUN rm ${PENTAHO_SERVER}/tomcat/lib/mysql-connector-java-5.1.17.jar 
